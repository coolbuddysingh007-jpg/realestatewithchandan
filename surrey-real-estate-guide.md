...
export default async function Page({ params }){
  ...
  const jsonLd = {
    ...
    "author": { "@type":"Person", "name":"Chandan Rathod" },
    "datePublished": frontmatter.date,
    "mainEntityOfPage": `https://realestatewithchandan.ca/blog/${params.slug}`
  };
      ...
      <section className="py-14">
        <Container>
          <div className="max-w-3xl">
            <div className="text-xs tracking-[0.35em] text-champagne-400">{frontmatter.category || "BLOG"}</div>
          ...
          <div className="mt-12 max-w-3xl rounded-2xl border border-white/10 bg-black/40 p-6">
            <div className="text-xs tracking-[0.3em] text-champagne-400">AUTHOR</div>
            <div className="mt-3 text-sm text-white/75">
              <strong>Chandan Rathod</strong> — Surrey & Langley Real Estate Advisor.
              <div className="mt-2">
                <a className="text-champagne-300 hover:text-champagne-200" href="mailto:chandanr@sutton.com">chandanr@sutton.com</a>
              </div>
            </div>
          </div>
        </Container>
